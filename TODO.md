...
~~1. **Automated test suite.**~~ Done — 117 tests under `tests/` (`pytest.ini` sets `pythonpath = src`). Covers `store.py`'s encryption round-trip, `graph.py`'s blast-radius/cut-vertex/cycle math, `metrics.py`'s four score components (including the `"breached"` gap), `remediation.py`'s six `_apply_fix` mutations, `hibp.py`'s k-anonymity matching logic (mocks `requests.get` — the suite never touches the real API), `export.py`'s HTML escaping, `graph_layout.py`'s force-directed layout, and all four importers including `keepass.py`'s Recycle Bin skip / nested-group recursion / both TOTP field conventions. `tests/conftest.py` has the shared fixtures: `reset_graph_state` (autouse, since `graph.py`'s singleton would otherwise leak between tests), `isolated_store` (points `store.py` at a `tmp_path` instead of the real per-OS app data dir), `no_network_hibp` (default all-clean, opt-in per test). `pytest` added to `requirements.txt`; `.github/workflows/test.yml` runs the suite on every push/PR, on all three OSes — no network access needed, no display/Tk dependency either (nothing here touches `ui/`).

~~2. **Graph visualization screen.**~~ Done — `ui/graph_view.py`, reachable via a new **View Graph** button on the Dashboard (never the main screen, per PRD section 13). Renders nodes/edges on a plain `tkinter.Canvas` (CustomTkinter has no canvas widget of its own) positioned by `src/graph_layout.py` — a pure-Python Fruchterman-Reingold force-directed layout, deliberately with no numpy/scipy dependency (networkx's own layout functions need numpy, which isn't in `requirements.txt`; graph.py's own docstring caps realistic scale at "tens to low hundreds of nodes," well within what a pure-Python simulation handles). Clicking a node shows its blast radius via the same `metrics.translate_to_consequences()` the Dashboard already uses — no new metric language invented for this screen. A small color-coded legend maps node types to friendlier labels ("Account", "Sign-in Method", ...) rather than showing raw internal type strings.

1. **Human QA pass.** Run `QA.md` for real on Windows, macOS, and Linux, with real (throwaway) exports from actual 1Password/Bitwarden/KeePass installs — every importer test so far (both scratch scripts and the `pytest` suite) has used hand-built synthetic fixtures, not a real export file from the real app. Now also covers a real (throwaway, definitely-breached, e.g. "password123") test password to confirm the HIBP check actually flags it end to end, a real offline run (disconnect network) to confirm the "could not check for breaches" message shows instead of a false negative, opening an exported HTML report in a real browser on each OS, and the Graph View screen (does the layout look reasonable on a real monitor, is a display-scaling issue visible, does clicking actually feel responsive — none of which the `pytest` suite can check since it never touches `ui/`).

## Deferred On Purpose

2. **API integrations** (GitHub, Google Workspace, Microsoft Entra) — explicitly pushed to the end per project decision. `ui/onboarding.py`'s "Connect Integration" panel is wired to a "Coming Soon" dialog and nothing else. When this starts: GitHub first (PRD section 7 lists it as the only MVP-scoped integration), needs OAuth or PAT-based auth, a client for 2FA/PAT/SSH-key/session status, and — the reason this is last — a real answer to the PRD's own risk note in section 10: an integration with write scope becomes the graph's highest-blast-radius node.

## Later Phases (PRD)

3. **Drift monitoring** (Roadmap v2) — periodic re-import, change detection, the "you haven't reviewed these 3 critical accounts in 90 days" review session. `metrics.py`'s staleness check (`_is_stale`, `STALE_AFTER_DAYS`) already exists as the underlying signal; this phase is the UI/scheduling layer on top of it. A periodic Pwned Passwords re-check (without re-importing) is a natural companion here, since a password can become breached after the fact.
4. **Published native installers** (Roadmap v4) — `build.spec` and `.github/workflows/build.yml` exist and build successfully locally, but the workflow triggers on a `v*` tag push and no such tag has ever been pushed. First real release is just pushing a tag once the above is far enough along to be worth shipping.
5. **The narrow automated-remediation exception** (PRD section 10: e.g. auto-revoking an expired GitHub PAT) — blocked on #2. Don't build this before there's an actual API client with a real minimal-scope token to revoke.
